...
import org.eclipse.jdt.internal.corext.refactoring.ConstantStore;

public final class O_JavaCopyProcessor extends CopyProcessor implements IReorgDestinationValidator {
	...
	public O_JavaCopyProcessor(ICopyPolicy copyPolicy) {
		fCopyPolicy= copyPolicy;
	}

	public O_JavaCopyProcessor(JavaRefactoringArguments refactoringArguments, RefactoringStatus status) {
	...
	public RefactoringStatus checkInitialConditions(IProgressMonitor pm) throws CoreException {
		...
		if(javaResources.length>0 && javaResources[0] instanceof IFile){
			...
			ConstantStore.setFile((IFile)javaResources[0]);
		}
	...
	public Change createChange(IProgressMonitor pm) throws CoreException {
		...
		Assert.isTrue(fCopyPolicy.getJavaElementDestination() != null || fCopyPolicy.getResourceDestination() != null);
		try {
			ConstantStore.setNewFilepath(fCopyPolicy.getJavaElementDestination().getCorrespondingResource().getLocation() +"/"+ ConstantStore.getFile().getName());
