 ...
 * @since 3.1
 */
public final class O_RefactoringExecutionStarter {
	...
	public static void startRenameRefactoring(final IJavaElement element, final Shell shell) throws CoreException {
		...
		if (support != null && support.preCheck().isOK()){
			...
			if(ConstantStore.getNewName()!=null && ConstantStore.getNewName().length() >1){
			 System.out.println("About to rename: "+ oldName +" to " + newPath+ConstantStore.getNewName());
				
			 SerialiseHelper.rename(oldName, newPath+ConstantStore.getNewName());
			}
	...
	private O_RefactoringExecutionStarter() {
